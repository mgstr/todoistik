...
- **so does Complete, for the same reason one level along.** It carried no
  `back`, so `back()` fell to the Referer — this page — and completing an
  action left you on the screen for editing it, with the button re-rendered as
  "Bring back" and nothing else moved: a press that read as having failed while
  the action really was done (design.md, "Editing items"). The doing screen's
  complete form carries `back` for exactly this reason (see "Doing"), and it
  is the same form posting to the same verb — so this is the page catching up
