...
- **the dated panels are built newest-first in Go, not reversed in the template.** `flow()` and `backlog()` count backwards from today, so the slice the template gets is already in reading order (design.md, "Dashboard"). The template has no ordering of its own to get wrong, and `Now` is still the first row it emits rather than something a reader has to look for at the end
