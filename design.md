...
The lists of tags and contexts (see "Contexts" and "Tags") are shown whole, as two sets of names with their counts, and narrowed by the same line every long view is narrowed by. It is the only place a name is removed, and it was also a form for adding one; it is now the line and nothing else, because a list that can be narrowed to the one name you are thinking of already says whether that name exists - and if it does not, the thing to do about it is the thing just typed.

- **the line asks about names, not about items.** `car` finds every name with *car* in it on both lists. `#car` asks the tag list only and `@car` the context list only, so the notation that writes a name is also how you say which list it is looked for on. Matching is the name filter's: case insensitive, substrings, every word present
...
- **a name nothing matches exactly can be created from the line.** Exactly, and not "the list is empty": `car` typed while `carpool` exists matches something, and a rule that waited for an empty list would make `car` impossible to create. The line has to be one name with its sigil - `#bike`, `@garage` or `@garage(Lidl)` - because a bare word does not say which list it belongs on, and a guess at that is the app deciding something that is yours. A name that differs from one already there only by case is not offered, being exactly the drift the lists exist to stop
