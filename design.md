...
- **Action**: it is done in a single step. The item is converted into an action and is written in the form an action is written in - the title starts with a verb and is self-descriptive; context and other optional fields may be filled in. The verb is asked for and not required: a title that does not open with one is marked and saved anyway (see "Verbs"). Two further things are settled here, and neither is a branch of its own:
